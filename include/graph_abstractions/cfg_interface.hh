...
public:
	...
	void generate_xdot(FILE* out = stdout){
		fprintf(out, "digraph cfg {\n");
		fprintf(out, "subgraph_fun_%s {\n", fun_name.c_str());
		for(auto &block : blocks) {
			fprintf(out, "bb_%s_%d[label=\"", fun_name.c_str(), block->block_id());
			if(block->block_id()==0){
				fprintf(out, "ENTRY: %s \n", fun_name.c_str());
			}
			block->dump(out);
			...
			for(BlockInterface<GraphType> *succ : block->successors()) {
				fprintf(out, "bb_%s_%d -> bb_%s_%d;\n", fun_name.c_str(), block->block_id(), fun_name.c_str(), succ->block_id());
			}
		}
		fprintf(out, "}\n");
		fprintf(out, "}\n");
